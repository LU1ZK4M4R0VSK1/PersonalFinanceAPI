...
description: '', amount: 0, type: 'expense', category_id: '', date: new
...
<div><Label>Descrição</Label><Input value={formData.description} onChange={e =>
setFormData({...formData, description: e.target.value})} required /></div>
<div><Label>Valor</Label><Input type="number" value={formData.amount}
...
<SelectContent>{categories.filter(c => c.type === formData.type).map(c =>
<SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
</Select>
</div>
<div><Label>Data</Label><Input type="date" value={formData.date} onChange={e =>
setFormData({...formData, date: e.target.value})} required /></div>
